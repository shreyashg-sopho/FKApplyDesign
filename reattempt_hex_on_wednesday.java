...
public class reattempt_hex_on_wednesday {
    ...
    public static String simulate_one_game (Player player1, Player PC,  Board game, int game_size, Board [][] Big_tic_tac)
    {
        ...
        while (true)
        {
            ...
            player1.make_move(curr,player1.sign);
...
{String sign;
    int row;
    int column;
    void make_move(String [][] curr, String sign)
    {
        Scanner input = new Scanner(System.in);
        ...
        if (check_valid_move(curr ,row, column , sign) != true)
        {
    ...
    boolean check_valid_move(String [][] curr , int row, int column, String sign)
    {
        if (curr[row][column] == "_")
        {curr [row] [column] = sign;
            return true;}
        else
            return false;
    }
